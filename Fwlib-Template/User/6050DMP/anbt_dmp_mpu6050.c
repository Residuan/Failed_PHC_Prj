...
u8 AnBT_DMP_MPU6050_DEV_CFG(void)		
{
	  ...
	  if (anbt_dmp_data[0]!=PRODUCT_WHOAMI) 
		{
			printf("WHO AM I error/n"); 
			printf("%c",anbt_dmp_data[0]);
			return 2;    
    }
		//
    ...
    if (anbt_dmp_rev)  //Congrats, these parts are better
		{
			...
			{
				printf("Unsupported version=1.0/n");  
        return 6;
			}
    } 
		else 
      ...
      if (!anbt_dmp_rev) 
			{
				printf("Product ID error/n"); 		//产品编号错误
				return 5;    
      } 
...
u8 AnBT_DMP_MPU6050_Init(void)	
{
	...
	printf("P-1,Finish DMP device init.\n"); 		//完成初始化
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("P-2,Finish DMP set sensor.\n");		//完成传感去设定
	else printf("P-2,DMP set sensor error.\n");
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("P-3,Finish DMP configure fifo.\n");		//完成fifo设置
	else printf("P-3,DMP configure fifo error.\n");
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ)) printf("P-4,Finish DMP set sample rate.\n");			//完成DMP采样率设置
	else printf("P-4,DMP set sample_rate error.\n");
	//
	if(!dmp_load_motion_driver_firmware())	printf("P-5,Finish DMP load motion driver firmware.\n");				//开启DMP功能
	else printf("P-5,DMP load motion driver firmware error.");
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) printf("P-6,Finish DMP set orientation.\n");		//初始方向设定偏差
	else printf("P-6,DMP set orientation error.\n");
	//
	if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
	DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
	DMP_FEATURE_GYRO_CAL))	
	printf("P-7,Finish DMP enable feature.\n");		//DMP使能
	else printf("P-7,DMP enable feature error.\n");
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) 
		printf("P-8,Finish DMP set fifo rate.\n");		//DMP  fifo速度
	else printf("P-8,DMP set fifo rate error.\n");
	//
	run_self_test();
	if(!mpu_set_dmp_state(1))
		printf("P-9,Finish DMP set dmp state.\n");				//DMP成功开启
	else printf("P-9,DMP set dmp state error.\n");					//Dmp启动失败
	//
	return 0;
}
